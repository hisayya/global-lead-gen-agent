...
### 9.2 发送限流与送达率（2026 实测基准）
- **每日上限**：单邮箱 25–30 封/天为安全上限（Gmail/Outlook 超 30 即限流）；MVP 用单个免费邮箱，控制在 ≤25–30/天。
- 首封纯文本、0–1 链接、无图片；真实发件人姓名+公司；随机延时；序列最多 3 封、间隔 5–7 天。
- 投诉率目标 <0.1%（红线 0.3%）；退信率 <2%。
- **可选升级**：独立发信域名（约 70–100 元/年）+ SPF/DKIM/DMARC，可显著提升送达率；非 MVP 必须，需用户另行决定。

### 9.3 合规（CAN-SPAM / GDPR / CASL）
- 每封含：真实发件人姓名与公司、物理/邮寄地址（footer）、清晰 opt-out；opt-out 10 个工作日内（力求当天）生效。
- 欧盟/英国（GDPR/PECR）：仅针对职业角色、用工作邮箱、保留「相关性」记录；不合并其他来源个人数据；已退订立即彻底删除。
- 加拿大（CASL）：仅发工作邮箱且公开列出、信息与角色直接相关；保留退订机制。
- `writer` 模板内置 opt-out 语句与寄件人身份/地址占位符。

### 9.4 LinkedIn / 社区
- 绝不自动化，仅 AI 出稿 + 人工点发，规避封号与 spam 规则。

### 9.5 Rust 工程最佳实践（来自生产复盘）
- **并发限流**：LLM 调用用 `tokio::sync::Semaphore` 限制同时在飞 5–10 个，避免 429（`governor` 亦可，但 Semaphore 零新增依赖）。
- **重试**：对 429/5xx 指数退避重试（手写，零新增依赖）。
- **边界**：设置合理 `max_tokens`；输入做长度校验与清洗；async 内只用 `tokio::time::sleep`，禁 `thread::sleep`。
- **可观测**：每次 agent/工具调用打 `tracing` span（名称、耗时、payload 大小、错误）。
- **护栏**：`max_steps` 上限；检测「重复调用同工具且结果相似」则强制收尾；高风险操作（写库/发信）加确认/白名单。
- **记忆**：事件流（JSONL）+ 定期摘要，不把全部历史无脑塞进 prompt。

### 9.6 抓取伦理与法律
- 抓取前查 `robots.txt` 与 ToS，遵守 Disallow 与 Crawl-delay；设置含联系方式的 descriptive User-Agent；尊重 429 + Retry-After。
- 速率：新域名 1 请求/3–5 秒，随机延时；缓存结果避免重复抓取；收到停止请求立即配合。
- **GDPR**：抓取欧盟居民个人数据（姓名/邮箱）需合法利益（legitimate interest）+ 书面 LIA，数据最小化、设保留 TTL、支持删除请求。
- **Google Maps 抓取特别提示**：Google ToS 原则上禁止自动化抓取地图数据。MVP 以**搜索引擎（DuckDuckGo 更宽松）+ GitHub + 直连企业官网**为主源，Google Maps 仅作「尽力而为、遇阻即停」的可选源，不对抗反爬、不绕过技术限制。
...
| 免费邮箱送达率/限流 | ≤25–30 封/邮箱/天、纯文本、opt-out、投诉率<0.1%；域名列为可选升级 |
| LinkedIn 封号 | 仅半自动出稿，绝不自动操作 |
| 抓取被反爬 | 限流+延时+多源兜底；失败不影响整体 |
| 合规风险（CAN-SPAM/GDPR/CASL） | 内置 opt-out + 寄件人身份/地址；仅工作邮箱、保留相关性记录、退订即删 |
| Google Maps 抓取违反 ToS | 以搜索引擎/GitHub/官网为主源，Maps 仅尽力而为、遇阻即停、不对抗反爬 |
| 抓取触及欧盟个人数据(GDPR) | legitimate interest + LIA + 数据最小化 + 保留 TTL + 删除请求支持 |
| LLM 429/5xx 限流 | Semaphore 并发限制(5–10) + 指数退避重试 |
