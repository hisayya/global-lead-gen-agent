...
fn load_queries(queries_file: Option<String>) -> Result<Vec<String>> {
    let path = queries_file.unwrap_or_else(|| {
        "docs/superpowers/specs/100-search-queries.md".to_string()
    });
...
async fn do_search(
    ...
    for chunk in queries.chunks(chunk_size) {
        ...
        for lead in leads {
            ...
            if store.lead_exists(&lead.email, &lead.company).unwrap_or(false) {
                continue;
            }
...
async fn diagnose_one(
    ...
    let score = scorer::Scorer::score(llm, &scored).await?;
    store.update_lead_score(id, score.score, score.qualified, &score.reason)?;
    ...
    if score.qualified {
        ...
        info!(company = %lead.company, score = score.score, "lead qualified + strategy generated");
    } else {
        info!(company = %lead.company, score = score.score, "lead not qualified");
    }
...
async fn do_send(
    ...
    if actual_limit <= 0 {
        info!(sent_today, limit = cfg.daily_send_limit, "daily limit reached");
        return Ok(());
    }
        ...
        Ok(results) => {
            for (email, classification) in &results {
                ...
                match classification.class.as_str() {
                    "interested" => {
                        warn!("🔥 HOT LEAD detected! Action: {}", classification.suggested_action);
                        println!("\n🔥 HOT LEAD!\n{}\n", email);
                    }
                    "objection" => {
